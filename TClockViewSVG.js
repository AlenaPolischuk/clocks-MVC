    ...
    render(model) {
        ...
        if (!this.svg) {
            ...
            for (let i = 1, deg = 30; i <= 12; i++, deg += 30) {
                ...
                this.circle.style.transformOrigin = "38% 38%"; //координаты точки, относительно которой будет происходить трансформация элемента.
                this.circle.style.transform = "rotate("+deg+"deg) translate(-40px, 70px)";
                this.svg.appendChild(this.circle);
            }
            for (let i = 1, deg = 30; i <= 12; i++, deg += 30) {    
                this.text = document.createElementNS('http://www.w3.org/2000/svg','text');
                this.text.textContent = i;
                this.text.setAttribute("x",(95+80*Math.sin(deg * Math.PI/180)) +  'px'); // цифры по кругу
