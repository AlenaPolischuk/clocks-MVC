class TClockViewDOM {
    ...
    // функция перемещения элементов (в нашем случае стрелок)
    // Статические методы часто используются для создания вспомогательных функций https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Classes/static
    static r(el, deg) { 
        //el.setAttribute('transform', 'rotate('+ deg +' 100 100)')

        el.style.transformOrigin = 'bottom right'; // Устанавливает координаты точки, относительно которой будет происходить трансформация элемента
    ...
    render (model) {
        ...
        if (!this.clock) {
            ...
            this.cityDiv = document.createElement('span');
            this.cityDiv.innerHTML = this.city;
            ...
            this.divInner.appendChild(this.cityDiv);
